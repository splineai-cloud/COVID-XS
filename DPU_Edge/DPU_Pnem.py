...
def function_handler(event, context):
    ...
    try:
        ...
        client.publish(
            ...
            payload=json.dumps(
                {
                    "message": " Sent from Greengrass Core running on platform: {}".format(my_platform) 
                                + " Input X-Ray Image = {}".format(imfile_ev) + " Inference Prediction= {}".format(output)
                }
            ),
        )
        logger.info("Publish Done: Image= {}".format(imfile_ev) + " Output= {}".format(output))
